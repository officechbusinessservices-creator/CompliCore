...
import { CheckCircle2, User, Bell, Shield, CreditCard, Globe } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [profile, setProfile] = useState({ name: "Jane Demo", email: "jane@example.com", phone: "+1 (555) 000-0000", timezone: "America/New_York", language: "English" });
  const [notifPrefs, setNotifPrefs] = useState({ newBooking: true, bookingReminder: true, guestMessage: true, payoutProcessed: true, reviewReceived: false, aiPricingUpdate: false });
      ...
      {activeTab === "profile" && (
        <div className="space-y-4">
          ...
          <div className="flex justify-end">
            <button onClick={save} className="px-6 py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 transition-opacity">
              Save changes
            </button>
          </div>
        </div>
      ...
      {activeTab === "security" && (
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <button onClick={save} className="px-4 py-2 rounded-lg border border-border text-sm font-medium hover:bg-accent transition-colors">
              Enable 2FA
            </button>
          </div>
          <div className="flex justify-end">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg border border-border">
              ...
              <button onClick={save} className="ml-auto text-xs text-primary hover:underline">Update</button>
            </div>
          </div>
        </div>
      )}
            ...
            {[
              ...
              <div key={i.name} className="flex items-center justify-between p-3 rounded-lg border border-border">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={save} className="text-xs text-primary hover:underline">
