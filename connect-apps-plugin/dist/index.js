    ...
    tools: [
        {
            ...
            description: "List all available app connectors for CompliCore (PMS providers, OTA channels, payment processors, smart locks, accounting, insurance). Optionally filter by category or status.",
            inputSchema: {
                type: "object",
                properties: {
                    category: {
        ...
        {
            name: "get_connector",
            description: "Get detailed information about a specific app connector including its status, required environment variables, and documentation URL.",
            inputSchema: {
                type: "object",
                properties: {
                    connectorId: {
        ...
        {
            name: "connect_app",
            description: "Activate a connection to an external application. For OAuth apps, provide the authorization code. For API-key apps, provide the key directly.",
            inputSchema: {
                type: "object",
                properties: {
                    connectorId: {
        ...
        {
            name: "trigger_sync",
            description: "Trigger a data sync with an external app (listings, bookings, availability, pricing, or full sync).",
            inputSchema: {
                type: "object",
                properties: {
                    connectorId: {
        ...
        {
            name: "check_env_vars",
            description: "Check which required environment variables are set (present) vs missing for a connector. Helps diagnose configuration issues without exposing secret values.",
            inputSchema: {
                type: "object",
                properties: {
                    connectorId: {
    ...
    try {
        switch (name) {
            ...
            case "get_connector": {
                ...
                const text = [
                    ...
                    recentSyncs.length === 0
                        ? "No sync history."
                        : recentSyncs
                            .map((s) => `- [${statusIcon(s.status)}] **${s.type}** — ${s.status} (${formatDuration(s.durationMs)}) at ${s.timestamp}` +
            ...
            case "get_sync_history": {
                ...
                const rows = records
                    .map((r) => `| ${statusIcon(r.status)} | \`${r.connectorId}\` | ${r.type} | ${r.status} | ${formatDuration(r.durationMs)} | ${r.recordsProcessed ?? "—"} | ${r.timestamp} |` +
