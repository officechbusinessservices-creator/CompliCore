...
export interface OrchestratorMetadata {
  fileType?: "image" | "audio" | "text" | "pdf" | "unknown";
}

export interface OrchestratorDecision {
  intent: ComplicoreIntent;
  module: string;
  reason: string;
}

export class ComplicoreOrchestrator {
  private readonly toolMap: Record<ComplicoreIntent, string> = {
    vision: "llama-ocr",
  ...
  routeRequest(input: string, metadata?: OrchestratorMetadata): OrchestratorDecision {
  ...
  private buildDecision(intent: ComplicoreIntent, reason: string): OrchestratorDecision {
    return {
      intent,
