...
export default async function propertiesRoutes(fastify: FastifyInstance) {
  fastify.get("/properties", async (request) => {
    ...
    try {
      const res = await query(
        "SELECT id, title, description, property_type, room_type, bedrooms, beds, bathrooms, max_guests, amenities, status, created_at FROM properties ORDER BY created_at DESC LIMIT 50"
      );
  ...
  fastify.post("/properties", async (request, reply) => {
    ...
    try {
      const res = await query(
        `INSERT INTO properties (host_id, address_id, title, description, property_type, room_type, bedrooms, beds, bathrooms, max_guests, amenities, status)
  ...
  fastify.get("/properties/:propertyId", async (request, reply) => {
    ...
    try {
      const res = await query(
        "SELECT id, title, description, property_type, room_type, bedrooms, beds, bathrooms, max_guests, amenities, status, created_at FROM properties WHERE id = $1",
        [propertyId]
      );
