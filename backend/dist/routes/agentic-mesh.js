...
exports.default = agenticMeshRoutes;
const zod_1 = require("zod");
const agentCardSchema = zod_1.z.object({
    "@context": zod_1.z.string().default("https://w3id.org/agentic/agent-card/v1"),
    id: zod_1.z.string().min(3),
    name: zod_1.z.string().min(3),
    did: zod_1.z.string().min(8),
    vendor: zod_1.z.string().min(2),
    signatureIssuer: zod_1.z.string().min(2),
    capabilities: zod_1.z.array(zod_1.z.string().min(2)).min(1),
    endpoint: zod_1.z.string().min(8),
    active: zod_1.z.boolean().default(true),
});
const handshakeSimulationSchema = zod_1.z.object({
    origin: zod_1.z.string().min(3),
    destination: zod_1.z.string().min(3),
    departureDate: zod_1.z.string().min(8),
    cabinClass: zod_1.z.enum(["ECONOMY", "PREMIUM_ECONOMY", "BUSINESS", "FIRST"]).default("BUSINESS"),
    maxBudgetUsd: zod_1.z.number().positive(),
    carbonCapTons: zod_1.z.number().positive(),
});
const guardrailCheckSchema = zod_1.z.object({
    actionType: zod_1.z.enum(["BOOKING", "PAYMENT", "ISSUANCE", "IDENTITY_ASSERTION"]),
    confidence: zod_1.z.number().min(0).max(100),
    transactionValueUsd: zod_1.z.number().nonnegative(),
    origin: zod_1.z.string().min(3),
    destination: zod_1.z.string().min(3),
    carrier: zod_1.z.string().min(2),
    carbonCapTons: zod_1.z.number().positive(),
    policyProfile: zod_1.z.enum(["STANDARD", "STRICT", "SOVEREIGN"]).default("SOVEREIGN"),
});
const zeroTouchSandboxSchema = zod_1.z.object({
    travelerId: zod_1.z.string().min(3),
    destinationHotelId: zod_1.z.string().min(3),
    destinationCity: zod_1.z.string().min(2).default("Dubai"),
    identityProvider: zod_1.z.enum(["SINGPASS", "UAE_PASS"]).default("SINGPASS"),
});
const biometricSettlementTriggerSchema = zod_1.z.object({
    bookingId: zod_1.z.string().min(6),
    travelerDid: zod_1.z.string().min(8),
    supplierId: zod_1.z.string().min(3),
    oidcProvider: zod_1.z.enum(["SINGPASS", "UAE_PASS"]),
    verificationHash: zod_1.z.string().min(8),
    escrowAmountUsd: zod_1.z.number().positive(),
    confidence: zod_1.z.number().min(0).max(100).default(99.5),
});
const disruptionSimulationSchema = zod_1.z.object({
    eventCode: zod_1.z.string().min(4).default("AUS-WX-L4"),
    affectedTravelers: zod_1.z.number().int().positive().default(12),
    corridor: zod_1.z.string().min(3).default("Austin"),
});
const genesisYieldDistributionSchema = zod_1.z.object({
    cycleId: zod_1.z.string().min(4).default("GENESIS-2026-02"),
    netRentalIncomeUsd: zod_1.z.number().positive().default(29600),
    applyLoyaltyMultiplier: zod_1.z.boolean().default(true),
});
const whalePartnershipDraftSchema = zod_1.z.object({
    whaleDid: zod_1.z.string().min(8),
    lockMonths: zod_1.z.number().int().min(1).max(60).default(18),
    mbfcUnitsLock: zod_1.z.number().int().positive().default(1500),
    usdcLock: zod_1.z.number().positive().default(1000000),
});
const journeyPlanSchema = zod_1.z.object({
    travelerId: zod_1.z.string().min(3),
    origin: zod_1.z.string().min(3),
    destination: zod_1.z.string().min(3),
    departureDate: zod_1.z.string().min(8),
    returnDate: zod_1.z.string().min(8).optional(),
    budget: zod_1.z.number().positive(),
    currency: zod_1.z.string().min(3).max(3).default("USD"),
    carbonCapKg: zod_1.z.number().positive().optional(),
    policyProfile: zod_1.z.enum(["STANDARD", "STRICT", "SOVEREIGN"]).default("SOVEREIGN"),
});
const a2aNegotiationSchema = zod_1.z.object({
    negotiationId: zod_1.z.string().min(4),
    buyerAgent: zod_1.z.string().min(3),
    sellerAgent: zod_1.z.string().min(3),
    serviceType: zod_1.z.enum(["FLIGHT", "LODGE", "GROUND"]),
    initialQuote: zod_1.z.number().positive(),
    maxBudget: zod_1.z.number().positive(),
    settlementMode: zod_1.z.enum(["FIAT", "STABLECOIN_INSTANT"]),
    policyProfile: zod_1.z.enum(["STANDARD", "STRICT", "SOVEREIGN"]).default("SOVEREIGN"),
});
const liabilityEvalSchema = zod_1.z.object({
    actionType: zod_1.z.enum(["BOOKING", "PAYMENT", "ISSUANCE", "IDENTITY_ASSERTION"]),
    actorType: zod_1.z.enum(["HUMAN", "AGENT"]),
    confidence: zod_1.z.number().min(0).max(100),
    jurisdiction: zod_1.z.string().min(2),
    requiresBiometric: zod_1.z.boolean().default(false),
    policyProfile: zod_1.z.enum(["STANDARD", "STRICT", "SOVEREIGN"]).default("SOVEREIGN"),
});
...
const toSegmentKey = (origin, destination, carrier) => `${origin.toUpperCase()}-${destination.toUpperCase()}-${carrier.toUpperCase()}`;
const getAgentTrust = (agent) => {
    let score = 30;
    if (agent.did.startsWith("did:"))
        score += 35;
    if (trustedIssuers.has(agent.signatureIssuer))
        score += 30;
    if (agent.active)
        score += 5;
...
async function agenticMeshRoutes(fastify) {
    ...
    fastify.post("/agentic/compliance/liability-eval", async (request, reply) => {
        ...
        const payload = parsed.data;
        const transactionValueUsd = Number(request.body?.transactionValueUsd ?? 0);
        const requiresHumanReview = (payload.actorType === "AGENT" && payload.confidence < hitlThreshold) || transactionValueUsd > 5000;
        ...
        return {
            ...
            hitl: {
                ...
                reason: transactionValueUsd > 5000
    ...
    fastify.post("/agentic/compliance/guardrail-check", async (request, reply) => {
        ...
        if (decision === "BLOCKED")
            guardrailCounters.blocked += 1;
        if (decision === "HITL_REQUIRED")
            guardrailCounters.hitlRequired += 1;
        if (decision === "AUTO_APPROVED")
            guardrailCounters.autoApproved += 1;
        return {
            status: "evaluated",
            decision,
            actionType: payload.actionType,
