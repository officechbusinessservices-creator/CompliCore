...
exports.default = economicRoutes;
const zod_1 = require("zod");
const crypto_1 = require("crypto");
const mintSimulationSchema = zod_1.z.object({
    assetId: zod_1.z.string().min(3),
    amount: zod_1.z.number().int().positive(),
    partition: zod_1.z.string().min(3),
    issuerDid: zod_1.z.string().min(8),
});
const ssiHandshakeSchema = zod_1.z.object({
    userId: zod_1.z.string().min(3),
    walletProvider: zod_1.z.enum(["apple", "google", "ledger", "custom"]),
    jurisdiction: zod_1.z.string().min(2),
    requestedCredentialType: zod_1.z.string().min(3),
});
const tradeSimulationSchema = zod_1.z.object({
    assetId: zod_1.z.string().min(3),
    partitionId: zod_1.z.number().int().positive(),
    buyerDid: zod_1.z.string().min(8),
    amount: zod_1.z.number().int().positive(),
    ssiVoucherValid: zod_1.z.boolean(),
    isSgResident: zod_1.z.boolean(),
    investorCredentialType: zod_1.z.enum(["StandardInvestor", "InstitutionalEntity"]).default("StandardInvestor"),
    currentUnits: zod_1.z.number().int().nonnegative().default(0),
});
const whitelistAddSchema = zod_1.z.object({
    userDid: zod_1.z.string().min(8),
    tier: zod_1.z.enum(["ACCREDITED", "INSTITUTIONAL"]),
    quota: zod_1.z.number().int().positive().max(100),
    memberId: zod_1.z.string().min(3),
});
const whitelistVerifySchema = zod_1.z.object({
    userDid: zod_1.z.string().min(8),
    memberId: zod_1.z.string().min(3),
});
const revenueDepositSchema = zod_1.z.object({
    assetId: zod_1.z.number().int().positive(),
    amount: zod_1.z.number().int().positive(),
    currency: zod_1.z.enum(["USDC", "SGD"]),
});
const revenueDistributeSchema = zod_1.z.object({
    assetId: zod_1.z.number().int().positive(),
    snapshotHolders: zod_1.z
        .array(zod_1.z.object({
        holderDid: zod_1.z.string().min(8),
        units: zod_1.z.number().int().nonnegative(),
    }))
        .min(1),
});
const manifestoSendSchema = zod_1.z.object({
    recipients: zod_1.z.array(zod_1.z.string().email()).min(1).max(100),
    allocationWindowHours: zod_1.z.number().int().positive().max(72).default(48),
});
const sentryEventSchema = zod_1.z.object({
    eventId: zod_1.z.string().min(4),
    action: zod_1.z.enum(["SSI_HANDSHAKE", "IDENTITY_VERIFIED", "MINT_COMMITTED", "ONBOARDING_ACTIVE"]),
    status: zod_1.z.enum(["PENDING", "SUCCESS", "COMMITTED", "ACTIVE", "FAILED"]),
    userRef: zod_1.z.string().min(3),
    didHash: zod_1.z.string().min(8).optional(),
    unitsClaimed: zod_1.z.number().int().nonnegative().optional(),
    investorCredentialType: zod_1.z.enum(["StandardInvestor", "InstitutionalEntity"]).optional(),
    dataResidency: zod_1.z.enum(["APAC", "EU", "US"]).optional(),
    latencyMs: zod_1.z.number().int().nonnegative().optional(),
});
const whalePauseSchema = zod_1.z.object({
    didHash: zod_1.z.string().min(8),
    mode: zod_1.z.enum(["LOCK", "UNLOCK"]),
    reason: zod_1.z.string().min(3).max(240).default("fair_distribution_control"),
});
const ccp302TriggerSchema = zod_1.z.object({
    force: zod_1.z.boolean().default(false),
    operatorNote: zod_1.z.string().min(3).max(240).optional(),
});
const finalSettlementSchema = zod_1.z.object({
    assetId: zod_1.z.string().min(3).default("SG-COMM-MBFC-001"),
    settlementAmountUsd: zod_1.z.number().int().positive().default(1000000),
    unitCount: zod_1.z.number().int().positive().default(1000),
    settlementRail: zod_1.z.enum(["wCBDC", "tUSD"]).default("wCBDC"),
    custodyVault: zod_1.z.string().min(3).default("Sovereign Custody Vault"),
    legalMirror: zod_1.z.boolean().default(true),
});
const capitalCallInitiateSchema = zod_1.z.object({
    targetAmountUsd: zod_1.z.number().int().positive().default(4000000),
    fundingWindowHours: zod_1.z.number().int().positive().max(168).default(48),
    settlementAsset: zod_1.z.enum(["wCBDC", "tUSD"]).default("wCBDC"),
});
const capitalCallPartnerUpdateSchema = zod_1.z.object({
    partnerId: zod_1.z.enum(["ANCHOR_A", "ANCHOR_B", "ANCHOR_C"]),
    status: zod_1.z.enum(["PENDING", "SIGNED", "FUNDED", "FAILED"]),
    receivedAmountUsd: zod_1.z.number().int().nonnegative().default(0),
    verification: zod_1.z.string().min(3).max(120).optional(),
    note: zod_1.z.string().min(3).max(240).optional(),
});
const privateCapitalUpdateSchema = zod_1.z.object({
    partnerId: zod_1.z.enum(["ANCHOR_C"]).default("ANCHOR_C"),
});
const whitelistRegistry = new Map();
const sbmBlockedIps = new Map();
const revenuePools = new Map();
...
const holdingsByDid = new Map();
const pausedMintDids = new Set();
let ccp302AutoTriggeredAt = null;
let finalSettlementRecord = null;
let retailMintingClosed = false;
let techHubAlphaMintedAt = null;
let capitalCallState = null;
...
async function economicRoutes(fastify) {
    const requireGovernanceRole = async (request, reply) => {
    ...
    const requireOpsRole = async (request, reply) => {
        const guard = fastify.requireRole;
        if (guard) {
    ...
    const hashMemberId = (memberId) => (0, crypto_1.createHash)("sha256").update(memberId).digest("hex");
    ...
    const getInstitutionalTier = (units) => {
        if (units >= 250)
            return "SOVEREIGN";
        if (units >= WHALE_THRESHOLD_UNITS)
            return "STRATEGIC";
        if (units >= 50)
            return "INSTITUTIONAL";
        return "ACCREDITED";
    };
    const classifyOwnershipSignal = (units, isInstitutional) => {
        if (isInstitutional && units >= 200)
            return { entityType: "Institutional Anchor", alertLevel: "INFO" };
    ...
    const buildGenesisSelloutReport = () => {
        ...
        const didTypeByLatestEvent = new Map();
        ...
        return {
            ...
            compliance: {
                ...
                blockedCapViolations: sentryEvents.filter((e) => e.action === "MINT_COMMITTED" &&
                    e.status === "FAILED" &&
                    e.investorCredentialType !== "InstitutionalEntity").length,
            },
    ...
    fastify.post("/economic/rwa/trade-simulation", async (request, reply) => {
        ...
        const { partitionId, ssiVoucherValid, isSgResident, buyerDid, amount, assetId, investorCredentialType, currentUnits, } = parsed.data;
    ...
    fastify.post("/economic/sentry/events", async (request, reply) => {
        ...
        if (event.action === "MINT_COMMITTED" &&
    ...
    fastify.get("/economic/sentry/first-ten", async () => {
        const verifiedUsers = new Set(sentryEvents
    ...
    fastify.get("/economic/whale/status", async () => {
        const watchlist = Array.from(holdingsByDid.entries())
            ...
            state: units >= WHALE_THRESHOLD_UNITS
                ? "WHALE_DETECTED"
    ...
    fastify.post("/economic/whale/pause-mint", async (request, reply) => {
        ...
        if (mode === "LOCK")
            pausedMintDids.add(didHash);
        if (mode === "UNLOCK")
            pausedMintDids.delete(didHash);
        return {
            status: "updated",
            didHash,
